...
public:
  ...
  AirportCounter(TIterator begin, TIterator end): AirportCounter(){
    while(begin != end){
      airports[static_cast<int>(*begin)]++;
      begin++;
    }
  }
  ...
  size_t Get(TAirport airport) const{
    return airports[static_cast<int>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport){
    airports[static_cast<int>(airport)]++;
  }
  ...
  void EraseAll(TAirport airport){
    airports[static_cast<int>(airport)] = 0;
  }
  ...
  Items GetItems() const{
    ...
    for(int i = 0; i < airports.size(); i++){
      items[i] = (make_pair(static_cast<TAirport>(i), airports[i]));
    }
    return items;
  }
