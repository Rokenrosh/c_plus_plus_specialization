...
struct AddressHasher {
  size_t operator() (const Address& address) const{
    size_t r1 = string_hash(address.city);
    size_t r2 = string_hash(address.street);
    size_t r3 = int_hash(address.building);
...
struct PersonHasher {
  size_t operator() (const Person& p) const{
    size_t r1 = string_hash(p.name);
    size_t r2 = int_hash(p.height);
