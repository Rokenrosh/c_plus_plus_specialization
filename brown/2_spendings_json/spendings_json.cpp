...
vector<Spending> LoadFromJson(istream& input) {
  ...
  for(auto& dict : doc.GetRoot().AsArray()){
    Spending spend;
    for(auto& el: dict.AsMap()){
      if(el.first == "amount"){
        spend.amount = el.second.AsInt();
      }
      else if(el.first == "category"){
        spend.category = el.second.AsString();
      }
    }
    result.push_back({spend.category, spend.amount});
  }
  return result;
}
