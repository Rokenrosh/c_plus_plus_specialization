...
struct MoneyState {
  ...
  double ComputeIncome() const { return earned - spent; }
};
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, MoneyState value) {
  ...
  if (ql <= l && r <= qr) {
    ...
    tree_add[v].spent += value.spent;
    tree_values[v].spent += value.spent * (r - l); 
    return;
  }
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double percent) {
  ...
  if (ql <= l && r <= qr) {
    ...
    tree_add[v].earned *= percent;
    tree_values[v].earned *= percent;
    return;
  }
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    auto const idx_from = ComputeDayIndex(Date::FromString(date_from_str));
    auto const idx_to = ComputeDayIndex(Date::FromString(date_to_str)) + 1;
    ...
    if (query_type == "ComputeIncome") {
      ...
      double value;
      cin >> value;
      const double earned = value / (idx_to - idx_from);
      Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, MoneyState {earned, 0.0});
    } else if (query_type == "Spend"){
      double value;
      cin >> value;
      const double spent = value / (idx_to - idx_from);
      Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, MoneyState {0.0, spent});
    }
  }
