...
public:
  ...
  MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
    return {origin.earned * tax_.factor + add_.delta.earned * segment.length(),
...
struct EarnRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{MoneyState{daily_income, 0.0}});
  }
...
struct SpendRequest: ModifyRequest{
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{MoneyState{0.0, daily_spending}});
  }
